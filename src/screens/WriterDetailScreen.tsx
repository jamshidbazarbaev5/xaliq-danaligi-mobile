...
const WriterDetailScreen = () => {
  ...
  console.log('bbook', fetchedBooks);
  // 2. Fetch data from the API
  useEffect(() => {
    const fetchBooks = async () => {
      const BASE_URL = 'https://xaliq-danaligi.uz:8000/api/mobile';
      setLoading(true);
      try {
        // Replace with your actual API endpoint and writer ID
        const response = await fetch(`${BASE_URL}/books/?author=${writer.id}`);
        const data = await response.json();
        setFetchedBooks(data.results);
      } catch (error) {
  ...
  const toggleAudio = async (item: Poem | Book) => {
    ...
    const audio =
      item.audio && typeof item.audio === 'object'
        ? Platform.OS === 'ios'
  ...
  const renderBookItem = ({item}: {item: any}) => {
    // Get the epub file URL from the book object
    const fileToUse = item.epub_file;

    // Ensure the URL is properly formatted and exists
    const epubUrl = fileToUse || null;

    // Debug log
    console.log('Debug epub file:', {
      originalFile: fileToUse,
      processedUrl: epubUrl,
      ...
      <TouchableOpacity
        ...
        onPress={() => {
          if (!epubUrl) {
            ...
            Alert.alert('Error', 'Epub file not available');
            return;
          }
          ...
          navigation.navigate('Writers', {
            ...
            params: {
              ...
              epubUrl: epubUrl,
              bookId: item.id,
              // Note: You'll need to implement reading progress logic
              // initialPage: readingProgress[item.id]?.position || 0,
            },
          });
        }}
        activeOpacity={0.7}>
        ...
        <View style={styles.bookInfo}>
          <Text style={[styles.bookTitle, {color: theme.textColor}]}>
            {item.title}
          </Text>
          <Text style={[styles.bookYear, {color: theme.secondaryTextColor}]}>
        ...
        <ScrollView
          ...
          <TouchableOpacity
            ...
            <Text
              ...
              {translations?.poems} ({fetchedBooks.length || 0})
            </Text>
          </TouchableOpacity>
        ...
        {activeTab === 'biography' ? (
            ...
            <Text
              ...
              {(() => {
                ...
                if (writer.biography_cyr || writer.biography_lat) {
                  return script === 'cyr'
                    ? writer.biography_cyr
                    : writer.biography_lat;
                }
          ...
          <View style={styles.booksContainer}>
            {loading ? (
              ...
              fetchedBooks.map(book => renderBookItem({item: book}))
            ) : (
              <Text
